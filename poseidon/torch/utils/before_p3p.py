...
def projection_one_point3D_to2D(point3D : Tensor, C : Tensor, R : Tensor,A : Tensor) -> Tensor :
    ...
    PI : Tensor = torch.cat((torch.eye(3, dtype=torch.float64),torch.zeros((3,1), dtype=torch.float64)),dim=1)  # (3,4)
    PI_batch : Tensor  = PI.unsqueeze(0).repeat(batch_size, 1, 1)  # (batch_size,3,4)
   

    Rt : Tensor  = torch.cat((R,C),dim=2)               # (batch_size,3,4)
    Rt = torch.cat((Rt,(torch.tensor([[0,0,0,1]], dtype=torch.float64)).repeat(batch_size,1,1)),dim=1)   # (batch_size,4,4)

    point3D_bis : Tensor  = torch.cat((torch.reshape(point3D,(batch_size,3,1)),(torch.tensor([[1]],dtype=torch.float64)).repeat(batch_size,1,1)),dim=1)   # (batch_size,4,1)
    point2D : Tensor = torch.matmul(A, torch.matmul(PI_batch, torch.matmul(Rt,point3D_bis)))  # (batch_size,3,1)
...
def projection_all_point3D_to2D(points3D : Tensor, C : Tensor, R : Tensor, A : Tensor) -> Tensor :
    ...
    points2D : Tensor = torch.empty(batch_size,4,2, dtype=torch.float64)  # Initialize tensor for 2D points
    for i in range(4) : 
