...
def P3P(points_3D : Tensor ,features_vectors : Tensor) -> Tensor : 
    ...
    tz : Tensor = torch.cross(f1,f2,dim=1)/ torch.norm(torch.cross(f1,f2,dim=1),dim=1, keepdim=True) 
    # (batch_size,3)
    ...
    nz : Tensor = torch.cross(nx,P3-P1,dim=1)/torch.norm(torch.cross(nx,P3-P1,dim=1), dim=1, keepdim=True)  # (batch_size,3)
    ...
    cosBeta : Tensor = (torch.sum(f1*f2,dim=1)/(torch.norm(f1,dim=1)*torch.norm(f2,dim=1))).unsqueeze(-1)  # tensor.dot(a,b) <=> tensor.sum(a*b)   # (batch_size,1)
    ...
    a3 : Tensor = 2 * p2**3 * d12 * b + 2 * phi2**2 * p2**3 * d12 * b - 2 * phi1 * phi2 * p2**3 * d12
    a2 : Tensor = - phi2**2 * p1**2 * p2**2 - phi2**2 * p2**2 * d12**2 * b**2 - phi2**2 * p2**2 * d12**2 + phi2**2 * p2**4 + phi1**2 * p2 **4 + 2 * p1 * p2**2 * d12 + 2 * phi1 * phi2 * p1 * p2**2 * d12 * b - phi1**2 * p1**2 * p2**2 + 2 * phi2**2 * p1 * p2**2 * d12 - p2**2 * d12**2 * b**2 - 2 * p1**2 * p2**2
    a1 : Tensor = 2 * p1**2 * p2 * d12 * b + 2 * phi1 * phi2 * p2**3 * d12 - 2 * phi2**2 * p2**3 * d12 * b - 2 * p1 * p2 * d12**2 * b
    a0 : Tensor = - 2 * phi1 * phi2 * p1 * p2**2 * d12 * b + phi2**2 * p2**2 * d12**2 + 2 * p1**3 * d12 - p1**2 * d12**2 + phi2**2 * p1**2 * p2**2 - p1**4 - 2 * phi2**2 * p1 * p2**2 * d12 + phi1**2 * p1**2 * p2**2 + phi2**2 * p2**2 * d12**2 * b**2

    # Computation of the roots of the polynomial
    roots : Tensor = polynomial_root_calculation_4th_degree_ferrari(a0,a1,a2,a3,a4) # (batch_size,4,2)
    ...
    for i in range(4):
        ...
        sin_teta : Tensor = torch.where(f3_T_positif,-torch.sqrt(1-cos_teta**2),torch.sqrt(1-cos_teta**2))

        cot_alpha : Tensor = ((phi1/phi2)*p1 + cos_teta*p2 -d12*b )/ ((phi1/phi2)*cos_teta* p2 - p1 + d12)
        ...
        C_estimate : Tensor = torch.stack([d12*cos_alpha*(sin_alpha*b + cos_alpha), d12*sin_alpha*cos_teta*(sin_alpha*b+cos_alpha), d12*sin_alpha*sin_teta*(sin_alpha*b+cos_alpha)],dim=1)  # (batch_size,3,1)
        # (batch_size,3,1)

        Q_row1 : Tensor = torch.stack([-cos_alpha, -sin_alpha*cos_teta, -sin_alpha*sin_teta], dim=-1)  
        Q_row2 : Tensor = torch.stack([sin_alpha, -cos_alpha*cos_teta, -cos_alpha*sin_teta],dim=-1)
        ...
        R_estimate : Tensor = torch.matmul(torch.matmul(torch.transpose(N,1,2),torch.transpose(Q, 1,2)),T)   # (batch_size,3,3)
...
def find_best_solution_P3P_batch(solutions : Tensor, points2D : Tensor, points3D : Tensor, A : Tensor) -> tuple[Tensor, Tensor, Tensor]: 
    '''
    ...
    for i in range(4):  # Iterate over the 4 solutions
        ...
        is_nan : Tensor  = torch.isnan(R).any(dim=(1, 2))  # Check if any element in R is NaN (batch_size,)
        ...
        erreur_totale : Tensor = torch.sum(distance, dim=1)  # Sum the distances for each solution (batch_size,)
    ...
    best_solutions : Tensor  = solutions[torch.arange(batch_size), index_min.squeeze(-1)] # (batch_size,3,4) 
    ...
    return R_opti, C_opti, value_min  # Return the optimal rotation matrix, position vector and the error of the best solution
